      ...
      messages: [
        {
          ...
          - Exact GCash UI elements, fonts, and layouts (2025 current version)
...
- App interface must match 2025 GCash design
...
- Impossible dates (dates before 2024, future dates beyond today)
...
ADVANCED CHECKS:
- Reference number validity: Must be exactly 9 OR 13 digits, all numbers
- Timestamp logic: Must be reasonable date (not before Jan 2024, not future)
- Amount formatting: Philippine peso standards (₱ symbol, comma separators)
- Phone status bar elements: Battery, signal, time must be consistent
- App version consistency: UI must match GCash 2025 current version

DATE VALIDATION:
- Year must be 2024 or 2025 only
- No dates before January 2024
- No future dates (beyond current date)
- Time format must be 12-hour with AM/PM

REFERENCE NUMBER VALIDATION:
- MUST be either 9 digits OR 13 digits
...
Be EXTREMELY strict. Flag ANY inconsistency as suspicious.`
    ...
    // Validate reference number format (9 or 13 digits)
    const refNumber = analysis.referenceNumber || '';
    const refNumberValid = /^\d{9}$|^\d{13}$/.test(refNumber);
    if (!refNumberValid && refNumber) {
      analysis.redFlags.push(`Invalid reference number format: Must be 9 or 13 digits, found: ${refNumber.length} characters`);
    }
    ...
    // Reference number validation
    if (!refNumberValid && refNumber) {
      analysis.redFlags.push(`Reference number invalid: Not 9 or 13 digits`);
      suspicionScore += 30;
    }
