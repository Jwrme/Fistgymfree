...
// Client sends feedback to coach
app.post('/api/client/send-feedback', async (req, res) => {
    try {
        const { 
            clientId,
            clientName, 
        ...
        // Validate required fields
        if (!clientId || !clientName || !coachId || !coachName || !feedback) {
            return res.status(400).json({ 
                success: false, 
                error: 'Missing required fields' 
            });
        ...
        // Find the coach to send feedback to
        const coach = await Coach.findById(coachId);
        if (!coach) {
            return res.status(404).json({ 
                success: false, 
        ...
        const feedbackNotification = {
            ...
            message: `Client ${clientName} sent you feedback: "${feedback}"`,
            timestamp: new Date(),
            read: false,
            feedbackData: {
                clientId,
        ...
        // Add notification to coach
        coach.notifications = coach.notifications || [];
        coach.notifications.push(feedbackNotification);
        await coach.save();

        console.log(`✅ Feedback sent from client ${clientName} to coach ${coachName}`);
        ...
        console.error('Error sending client feedback:', error);
        res.status(500).json({ success: false, error: 'Server error.' });
    }
});
