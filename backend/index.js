...
const { buildVerificationEmail, buildWelcomeEmail } = require('./emailTemplates');
const { verifyReceiptWithAI, checkDuplicateReceipt, basicReceiptValidation } = require('./receiptValidator');
  ...
  bookings: [
    {
      ...
      referenceNumber: String, // Reference number for payment tracking
      // Package booking fields
      isPackage: { type: Boolean, default: false },
      packageType: String,
      packageSessions: Number,
      packagePrice: Number,
      packageId: String // Unique identifier for package bookings
    }
  ],
  bookingHistory: [
    {
...
const bookingSchema = new mongoose.Schema({
  ...
  referenceNumber: { type: String } // Reference number for payment tracking
});
  ...
  if (user.verificationCode === code) {
    ...
    try {
      ...
      await transporter.sendMail({
        from: 'SenJitsu <' + process.env.EMAIL_USER + '>',
        to: user.email,
        subject,
        html,
        text
...
const generateReferenceNumber = () => {
  const prefix = 'FG'; // Fist Gym prefix
  const timestamp = Date.now().toString().slice(-6); // Last 6 digits of timestamp
  const random = Math.random().toString(36).substring(2, 8).toUpperCase(); // 6 random alphanumeric characters
...
app.post('/api/book', async (req, res, next) => {
  try {
    ...
    const bookingWithPayment = {
      ...
      referenceNumber: paymentProof ? generateReferenceNumber() : null
    };
    ...
    if (paymentProof) {
      const newBookingDoc = new Booking({
        ...
        referenceNumber: bookingWithPayment.referenceNumber
      });
    ...
    res.json({
      ...
      message: paymentProof ?
        `Booking created with reference number ${newBooking.referenceNumber}. Please wait for payment verification.` :
...
app.post('/api/coaches', async (req, res, next) => {
  ...
  try {
    ...
    // Send welcome email with credentials to the new coach
    try {
      const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
      ...
      const subject = 'Welcome to FistGym - Your Coach Account Details';
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8fff8; border: 2px solid #2ecc40; border-radius: 12px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #145a32; font-family: 'Courier New', Courier, monospace; margin: 0;">🥊 FISTGYM</h1>
            <h2 style="color: #2ecc40; margin: 10px 0;">Welcome to the Team!</h2>
          </div>
          
          <div style="background: white; padding: 25px; border-radius: 8px; box-shadow: 0 2px 8px rgba(44,204,64,0.1);">
            <p style="color: #145a32; font-size: 16px; margin-bottom: 20px;">
              Hi <strong>${firstname} ${lastname}</strong>,
            </p>
            
            <p style="color: #333; line-height: 1.6; margin-bottom: 20px;">
              Congratulations! Your coach account has been successfully created at FistGym. We're excited to have you join our team of professional trainers.
            </p>
            
            <div style="background: #f0f8f0; padding: 20px; border-radius: 8px; border-left: 4px solid #2ecc40; margin: 20px 0;">
              <h3 style="color: #145a32; margin-top: 0;">Your Login Credentials:</h3>
            ...
            <p style="color: #333; line-height: 1.6; margin: 20px 0;">
              You can now log in to your coach portal to:
            </p>
            
            <ul style="color: #333; line-height: 1.8; padding-left: 20px;">
              <li>Manage your class schedules and availability</li>
              <li>View and interact with your students</li>
              <li>Update your profile and specialties</li>
              <li>Track your coaching sessions</li>
            </ul>
            
            <div style="text-align: center; margin: 30px 0;">
              <p style="color: #145a32; font-size: 18px; font-weight: bold;">
                Ready to start your coaching journey with us!
              </p>
            </div>
            ...
            <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
              <p style="color: #145a32; font-weight: bold; margin: 0;">Welcome to FistGym Family! 🥊</p>
              <p style="color: #666; font-size: 14px; margin: 5px 0 0 0;">Let's train champions together!</p>
            </div>
          </div>
        </div>
      `;
      ...
      const text = `
        Welcome to FistGym, ${firstname} ${lastname}!
        
        Your coach account has been successfully created.
        ...
        Welcome to the FistGym family!
      `;

      const mailOptions = {
        from: 'FistGym <' + process.env.EMAIL_USER + '>',
        to: email,
        subject: subject,
        html: html,
        text: text
      ...
      console.error('❌ Error sending welcome email:', emailError);
      // Don't fail the coach creation if email fails
    }

    console.log('✅ Coach account created successfully with welcome email sent.');

    res.status(201).json({ success: true, message: 'Coach added successfully! Welcome email sent to coach.' });
  } catch (err) {
...
// GET payroll history for a specific coach
app.get('/api/payroll/history/:coachId', async (req, res) => {
  try {
    const { coachId } = req.params;
    console.log('📋 Fetching payroll history for coach:', coachId);
    ...
    // Fetch payroll history for the specific coach
    const payrollHistory = await Payroll.find({ coachId })
      .populate('coachId', 'firstname lastname')
      .sort({ paymentDate: -1 });

    console.log('📋 Found payroll records for coach:', payrollHistory.length);
    ...
    const formattedHistory = payrollHistory.map(payment => {
      ...
      return {
        ...
        totalClients: payment.totalClients || 0,
        totalRevenue: payment.totalRevenue || 0,
        coachShare: payment.coachShare || payment.amount || 0,
        attendanceSummary: payment.attendanceSummary || {
          totalDaysPresent: 0,
          totalDaysAbsent: 0,
          totalDaysMarked: 0
        ...
        processedBy: payment.processedBy || 'admin'
      };
    });

    console.log('📋 Formatted history records for coach:', formattedHistory.length);
    res.json(formattedHistory);
  } catch (error) {
    console.error('❌ Error fetching coach payroll history:', error);
    res.status(500).json({
      error: 'Failed to fetch coach payroll history',
      ...
      user.bookings.forEach(booking => {
        if (booking.coachId === coachId && booking.paymentStatus === 'verified') {
          verifiedBookings.push({
            ...
            referenceNumber: booking.referenceNumber, // Include reference number
...
app.post('/api/ratings', async (req, res) => {
  try {
    const { userId, username, studentName, coachId, coachName, rating, comment, bookingId, date, time, class: className } = req.body;

    // Validate required fields
    if (!userId || !username || !studentName || !coachId || !coachName || !rating || !date || !time || !className) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required fields' 
      });
    ...
    console.log(`✅ New rating submitted: ${studentName} rated ${coachName} ${rating}/5 stars`);
...
app.get('/api/ratings/coach/:coachId', async (req, res) => {
  try {
    const { coachId } = req.params;
    
    const ratings = await Rating.find({ coachId })
      .sort({ createdAt: -1 })
      .limit(50); // Limit to latest 50 ratings
...
// Get ratings summary for all coaches
app.get('/api/ratings/summary', async (req, res) => {
  try {
    const ratingSummary = await Rating.aggregate([
      {
        $group: {
          ...
          totalRatings: { $sum: 1 },
          ratings: { $push: { rating: '$rating', comment: '$comment', studentName: '$studentName', createdAt: '$createdAt' } }
        }
      },
      {
        $project: {
          ...
          totalRatings: 1,
          ratings: { $slice: ['$ratings', 5] } // Latest 5 ratings
        }
      },
      {
    ...
    res.status(500).json({ 
      success: false, 
      error: 'Server error while fetching ratings summary' 
    });
  }
});
...
app.delete('/api/coach-notifications', async (req, res) => {
  ...
  try {
    // Find the coach and remove notifications from the notifications array
    const coach = await Coach.findById(coachId);
...
app.put('/api/coach-notifications/read', async (req, res) => {
  ...
  if (!coachId || !notifIds || !Array.isArray(notifIds)) return res.status(400).json({ success: false, error: 'coachId and notifIds required' });
  try {
    // Find the coach and mark notifications as read in the notifications array
    const coach = await Coach.findById(coachId);
    if (!coach) return res.status(404).json({ success: false, error: 'Coach not found' });
...
app.post('/api/membership-application', async (req, res) => {
  try {
    const { name, email, userId, proof, userReferenceNumber } = req.body;
    if (!proof) return res.status(400).json({ success: false, message: 'Proof of payment is required.' });

    console.log('🔍 [MEMBERSHIP] User-entered reference number:', userReferenceNumber);
    ...
    const duplicateCheck = await checkDuplicateReceipt(proof, MembershipApplication);
    if (duplicateCheck.isDuplicate) {
      console.log('⚠️ DUPLICATE RECEIPT DETECTED:', duplicateCheck.message);
      return res.status(400).json({
        success: false,
        message: 'This receipt has already been used for a previous membership application.',
        details: duplicateCheck.message
      });
    }

    // 🤖 AI RECEIPT VERIFICATION
    const AI_ENABLED = process.env.ENABLE_AI_VERIFICATION === 'true';
    const AI_TEST_MODE = process.env.AI_TEST_MODE !== 'false';
    ...
    if (AI_ENABLED) {
      try {
        console.log('🤖 [MEMBERSHIP] AI verification enabled, analyzing receipt...');
        console.log('🤖 [MEMBERSHIP] Test mode:', AI_TEST_MODE ? 'ON (logging only)' : 'OFF (enforcing)');

        aiVerification = await verifyReceiptWithAI(proof, 1000, {
          userId,
          name,
          email,
        ...
        console.log('🤖 [MEMBERSHIP] AI Result:', {
          recommendation: aiVerification.recommendation,
          suspicionScore: aiVerification.suspicionScore,
          confidence: aiVerification.confidence,
          redFlags: aiVerification.redFlags
        });

        // In TEST MODE: Log but don't enforce
        if (AI_TEST_MODE) {
          console.log('🧪 [MEMBERSHIP] TEST MODE: AI result logged but not enforced');
          console.log('🧪 [MEMBERSHIP] Would have:', aiVerification.recommendation);
          applicationStatus = 'pending'; // Always pending in test mode
        } else {
          // PRODUCTION MODE: Enforce AI decision
          if (aiVerification.autoReject) {
            console.log('🚫 [MEMBERSHIP] AI REJECTED: Auto-rejecting application');
            return res.status(400).json({
              success: false,
              message: 'Receipt verification failed. Please upload a valid payment receipt.',
              details: aiVerification.redFlags,
              aiConfidence: aiVerification.confidence
            });
          }

          // Set status based on AI recommendation
          applicationStatus = aiVerification.suggestedStatus === 'verified' ? 'approved' : 'pending';
          console.log('✅ [MEMBERSHIP] AI Status:', applicationStatus);
        }

      } catch (aiError) {
        console.error('❌ [MEMBERSHIP] AI verification error:', aiError.message);
        console.log('⚠️ [MEMBERSHIP] Falling back to manual review');
        applicationStatus = 'pending';
      }
    } else {
      console.log('ℹ️ [MEMBERSHIP] AI verification disabled');
    }

    // 🆕 VALIDATE REFERENCE NUMBER MATCH
    let referenceNumberMatch = false;
    if (aiVerification && userReferenceNumber) {
      const aiRefNumber = aiVerification.extractedData?.referenceNumber?.toString().trim();
      const userRefNumber = userReferenceNumber.toString().trim();

      console.log('🔍 [MEMBERSHIP REF CHECK] User entered:', userRefNumber);
      console.log('🔍 [MEMBERSHIP REF CHECK] AI extracted:', aiRefNumber);

      if (aiRefNumber && aiRefNumber === userRefNumber) {
        console.log('✅ [MEMBERSHIP REF MATCH] Reference numbers match!');
        referenceNumberMatch = true;
        aiVerification.suspicionScore = Math.max(aiVerification.suspicionScore - 15, 0);
      } else if (aiRefNumber && aiRefNumber !== userRefNumber) {
        console.log('❌ [MEMBERSHIP REF MISMATCH] Reference numbers do not match!');
        referenceNumberMatch = false;
        aiVerification.redFlags.push(`CRITICAL: Reference number mismatch - User entered ${userRefNumber}, receipt shows ${aiRefNumber}`);
        aiVerification.suspicionScore = Math.min(aiVerification.suspicionScore + 50, 100);

        if (applicationStatus === 'approved') {
          applicationStatus = 'pending'; // Downgrade to manual review
        }

        if (!AI_TEST_MODE) {
          console.log('🚫 [MEMBERSHIP] REJECTING due to reference number mismatch');
          return res.status(400).json({
            success: false,
            message: 'Reference number does not match the receipt. Please check and try again.',
            details: [`You entered: ${userRefNumber}`, `Receipt shows: ${aiRefNumber}`]
          });
        }
      }
    }
    ...
    const application = new MembershipApplication({
      ...
      receiptHash: duplicateCheck.imageHash,
      aiVerified: !!aiVerification,
      aiConfidence: aiVerification?.confidence,
      aiSuspicionScore: aiVerification?.suspicionScore,
      aiRecommendation: aiVerification?.recommendation,
      aiExtractedData: aiVerification?.extractedData,
      aiRedFlags: aiVerification?.redFlags,
      aiReasoning: aiVerification?.reasoning,
      aiVerifiedAt: aiVerification?.verifiedAt,
      // 🆕 Reference number validation
...
const paymentSchema = new mongoose.Schema({
  ...
  // Multi-slot booking fields
  isMultiSlotBooking: { type: Boolean, default: false },
  multiSlotTotal: Number, // Total amount for all slots in the multi-slot booking
  multiSlotCount: Number, // Number of slots in the multi-slot booking
  // 🤖 AI Verification fields
  receiptHash: String, // MD5 hash for duplicate detection
  aiVerified: Boolean, // Whether AI verification was used
  aiConfidence: Number, // AI confidence score (0-100)
  aiSuspicionScore: Number, // Suspicion score (0-100)
  aiRecommendation: String, // APPROVE, MANUAL_REVIEW, or REJECT
  aiExtractedData: Object, // Data extracted by AI
  aiRedFlags: [String], // Red flags found by AI
  aiReasoning: String, // AI's reasoning
  aiVerifiedAt: Date, // When AI verification ran
  // 🆕 Reference Number Validation
  userReferenceNumber: String, // Reference number entered by user
  referenceNumberMatch: Boolean // Whether user's ref matches AI-extracted ref
});
...
app.post('/api/updatePayment', async (req, res) => {
  try {
    ...
    const { userId, coachId, date, time, paymentProof, amount, originalAmount, membershipDiscount, hasMembershipDiscount, qrCodeType, className, userReferenceNumber, isMultiSlotBooking, multiSlotTotal, multiSlotCount, isPackage, packageType, packageSessions, packagePrice, packageId } = req.body;

    console.log('🔍 User-entered reference number:', userReferenceNumber);
    
    // Log multi-slot booking info
    if (isMultiSlotBooking) {
      console.log('🎯 Multi-slot booking detected:', {
        multiSlotCount,
        multiSlotTotal,
    ...
    // 🔍 DUPLICATE CHECK - Skip for multiple session bookings with same packageId
    let duplicateCheck = { isDuplicate: false, imageHash: null };
    
    if (isPackage && packageId) {
      console.log('🔗 Multi-session booking detected, checking for existing sessions with same packageId...');
      const existingPackagePayments = await Payment.find({ packageId: packageId });
      console.log(`📦 Found ${existingPackagePayments.length} existing payments with packageId: ${packageId}`);
      
      // Allow duplicate receipts for same package, but check if this specific session already exists
      const existingSession = existingPackagePayments.find(p => 
        p.coachId === coachId && p.date === date && p.time === time
      );
      
      if (existingSession) {
        console.log('⚠️ DUPLICATE SESSION DETECTED in same package:', existingSession._id);
        return res.status(400).json({
          success: false,
          error: 'This session has already been submitted for this package.',
          details: `Session on ${date} at ${time} with coach ${coachId} already exists`
        });
      }
      
      // For package bookings, generate hash manually for tracking but don't check for duplicates
      const imageHash = crypto.createHash('md5').update(paymentProof).digest('hex');
      duplicateCheck = { isDuplicate: false, imageHash: imageHash };
    } else {
      // Regular duplicate check for non-package bookings
      console.log('🔍 Checking for duplicate receipts...');
      duplicateCheck = await checkDuplicateReceipt(paymentProof, Payment);
      if (duplicateCheck.isDuplicate) {
        console.log('⚠️ DUPLICATE RECEIPT DETECTED:', duplicateCheck.message);
        return res.status(400).json({
          success: false,
          error: 'This receipt has already been used for a previous payment.',
          details: duplicateCheck.message
        });
      }
    }
    ...
    // 🤖 AI RECEIPT VERIFICATION
    const AI_ENABLED = process.env.ENABLE_AI_VERIFICATION === 'true';
    const AI_TEST_MODE = process.env.AI_TEST_MODE !== 'false'; // Default to test mode for safety

    let aiVerification = null;
    let paymentStatus = 'for approval'; // Default status

    if (AI_ENABLED) {
      try {
        console.log('🤖 AI verification enabled, analyzing receipt...');
        console.log('🤖 Test mode:', AI_TEST_MODE ? 'ON (logging only)' : 'OFF (enforcing)');

        aiVerification = await verifyReceiptWithAI(paymentProof, amount, {
          userId,
          coachId,
          date,
          time,
        ...
        // In TEST MODE: Log but don't enforce
        if (AI_TEST_MODE) {
          console.log('🧪 TEST MODE: AI result logged but not enforced');
          console.log('🧪 Would have:', aiVerification.recommendation);
          paymentStatus = 'for approval'; // Always pending in test mode
        } else {
          // PRODUCTION MODE: Enforce AI decision
          if (aiVerification.autoReject) {
            console.log('🚫 AI REJECTED: Auto-rejecting payment');
            return res.status(400).json({
              success: false,
              error: 'Receipt verification failed. Please upload a valid payment receipt.',
              details: aiVerification.redFlags,
              aiConfidence: aiVerification.confidence
            });
          }
          ...
          // Set status based on AI recommendation
          paymentStatus = aiVerification.suggestedStatus;
          console.log('✅ AI Status:', paymentStatus);
        }

      } catch (aiError) {
        console.error('❌ AI verification error:', aiError.message);
        console.log('⚠️ Falling back to manual review');
        paymentStatus = 'for approval';
        // Don't fail the whole request if AI fails
      }
    } else {
      console.log('ℹ️ AI verification disabled');
    }
    ...
    if (aiVerification && userReferenceNumber) {
      ...
      console.log('🔍 [REF CHECK] User entered:', userRefNumber);
      console.log('🔍 [REF CHECK] AI extracted:', aiRefNumber);

      if (aiRefNumber && aiRefNumber === userRefNumber) {
        console.log('✅ [REF MATCH] Reference numbers match!');
        referenceNumberMatch = true;
        // Reduce suspicion score as reward for matching
        aiVerification.suspicionScore = Math.max(aiVerification.suspicionScore - 15, 0);
      } else if (aiRefNumber && aiRefNumber !== userRefNumber) {
        console.log('❌ [REF MISMATCH] Reference numbers do not match!');
        referenceNumberMatch = false;
        aiVerification.redFlags.push(`CRITICAL: Reference number mismatch - User entered ${userRefNumber}, receipt shows ${aiRefNumber}`);
        aiVerification.suspicionScore = Math.min(aiVerification.suspicionScore + 50, 100);

        // Update status to manual review due to mismatch
        if (paymentStatus === 'verified') {
          paymentStatus = 'for approval'; // Downgrade to manual review
        }

        // If mismatch in production mode, reject
        if (!AI_TEST_MODE) {
          console.log('🚫 REJECTING due to reference number mismatch');
          return res.status(400).json({
            success: false,
            error: 'Reference number does not match the receipt. Please check and try again.',
            details: [`You entered: ${userRefNumber}`, `Receipt shows: ${aiRefNumber}`]
          });
        }
      }
    ...
    if (DEBUG_MODE) {
      ...
      console.log('🔍 [PAYMENT CREATION] Status:', paymentStatus);
      console.log('🔍 [PAYMENT CREATION] Reference match:', referenceNumberMatch);
    }
    ...
    const newPayment = new Payment({
      ...
      className: resolvedClassName,
      paymentProof: paymentProof,
      status: paymentStatus, // Use AI-determined status
      ...
      // Multi-slot booking information
      isMultiSlotBooking: isMultiSlotBooking || false,
      multiSlotTotal: multiSlotTotal,
      multiSlotCount: multiSlotCount,
      // Package information for multiple session bookings
    ...
    // 🔥 CREATE OR UPDATE USER'S BOOKING
    let user = null;
    let hasUpdates = false;
    try {
      console.log('🔄 Creating/updating user booking...');
      ...
      if (user) {
        ...
        // First, try to find existing booking to update
        console.log('🔍 Looking for existing booking with:', { coachId, date, time });
        ...
        user.bookings.forEach((booking, index) => {
          ...
          if (coachMatches && dateMatches && timeMatches) {
            console.log('✅ Found existing booking! Updating status...');
            booking.paymentStatus = 'pending';
            booking.paymentProof = paymentProof;
            booking.paymentDate = new Date();
            // Generate reference number if not already present
            if (!booking.referenceNumber) {
              booking.referenceNumber = generateReferenceNumber();
            }
            // Add package fields for multiple session bookings
            if (isPackage) {
              booking.isPackage = true;
              booking.packageType = packageType;
              booking.packageSessions = packageSessions;
              booking.packagePrice = packagePrice;
              booking.packageId = packageId;
            }
            hasUpdates = true;
            foundExisting = true;
            console.log(`✅ Updated existing booking for ${user.username}: ${booking.coachName} (${booking.date}) to pending with reference: ${booking.referenceNumber}${isPackage ? ` (Package: ${packageType})` : ''}`);
          }
        });

        // If no existing booking found, create a new one (for multiple session bookings)
        if (!foundExisting) {
          console.log('📝 No existing booking found, creating new booking entry...');
          const newBooking = {
            coachId: coachId,
            coachName: coachName,
            date: date,
            time: time,
            class: resolvedClassName || 'Boxing',
            paymentStatus: 'pending',
            paymentProof: paymentProof,
            paymentDate: new Date(),
            referenceNumber: generateReferenceNumber()
          ...
          // Add package fields for multiple session bookings
          if (isPackage) {
            newBooking.isPackage = true;
            newBooking.packageType = packageType;
            newBooking.packageSessions = packageSessions;
            newBooking.packagePrice = packagePrice;
            newBooking.packageId = packageId;
          }

          user.bookings.push(newBooking);
          hasUpdates = true;
          console.log(`✅ Created new booking for ${user.username}: ${coachName} (${date}) at ${time}${isPackage ? ` (Package: ${packageType})` : ''}`);
        }
        ...
        if (hasUpdates) {
          ...
          if (unpaidBookings.length > 0) {
            ...
            // Generate reference number if not already present
            if (!mostRecentUnpaid.referenceNumber) {
              mostRecentUnpaid.referenceNumber = generateReferenceNumber();
            }

            user.markModified('bookings');
            await user.save();
            console.log('💾 Fallback: Updated most recent unpaid booking to pending with reference:', mostRecentUnpaid.referenceNumber);
            hasUpdates = true;
          } else {
        ...
        const discountMessage = hasMembershipDiscount ?
          ` with member discount (₱${parseFloat(originalAmount).toFixed(2)} - ₱${parseFloat(membershipDiscount).toFixed(2)} = ₱${parseFloat(amount).toFixed(2)})` : '';
    ...
    try {
      ...
      const notificationUser = await User.findOne({ username: userId });
      const userName = notificationUser ? `${notificationUser.firstname} ${notificationUser.lastname}` : userId;
      ...
      const discountInfo = hasMembershipDiscount ?
        ` (Original: ₱${parseFloat(originalAmount).toFixed(2)}, Member discount: -₱${parseFloat(membershipDiscount).toFixed(2)}, Final: ₱${parseFloat(amount).toFixed(2)}${qrInfo})` :
        ` (Amount: ₱${parseFloat(amount).toFixed(2)}${qrInfo})`;
    ...
    // Get the reference number from the updated booking and update the Payment document
    let referenceNumber = null;
    if (hasUpdates && user) {
      const updatedBooking = user.bookings.find(booking => {
        const coachMatches = booking.coachId === coachId ||
          booking.coachName?.includes(coachName) ||
          coachName?.includes(booking.coachName);
        const dateMatches = booking.date === date;
        const timeMatches = booking.time === time;
        return coachMatches && dateMatches && timeMatches;
      });
      referenceNumber = updatedBooking?.referenceNumber;

      // Update the Payment document with the reference number
      if (referenceNumber && savedPayment) {
        try {
          await Payment.findByIdAndUpdate(savedPayment._id, { referenceNumber: referenceNumber });
          console.log('✅ Updated Payment document with reference number:', referenceNumber);
        } catch (error) {
          console.error('❌ Error updating Payment document with reference number:', error);
        }
      }
    }

    // Response based on AI verification status
    let responseMessage = 'Payment proof submitted successfully!';
    if (aiVerification) {
      if (paymentStatus === 'verified') {
        responseMessage = '✅ Payment verified automatically! Your booking is confirmed.';
      } else if (aiVerification.needsManualReview) {
        responseMessage = '⏳ Payment submitted for review. You will be notified once verified.';
      }
    } else {
      responseMessage = '⏳ Payment proof submitted. Please wait for admin verification.';
    }
    ...
    const errorDetails = process.env.NODE_ENV === 'development' ? error.stack : 'Server error during payment submission.';
  ...
  try {
    ...
    user.bookings.forEach((booking, index) => {
      ...
      if (booking.coachId === coachId &&
        ...
        if (paymentProof) {
          booking.paymentProof = paymentProof;
          // Generate reference number if not already present and payment is being submitted
          if (!booking.referenceNumber && paymentStatus === 'pending') {
            booking.referenceNumber = generateReferenceNumber();
          }
        }
        booking.paymentDate = new Date();
        hasUpdates = true;
        console.log(`✅ Updated booking status for ${user.username}: ${booking.coachName} (${booking.date}) to ${paymentStatus}${booking.referenceNumber ? ` with reference: ${booking.referenceNumber}` : ''}`);
      }
    });
    ...
    const enrichedPayments = await Promise.all(
      payments.map(async (payment) => {
        ...
        return {
          ...
          referenceNumber: payment.referenceNumber, // Include reference number
          ...
          // Include membership discount fields
          originalAmount: payment.originalAmount,
          membershipDiscount: payment.membershipDiscount,
          hasMembershipDiscount: payment.hasMembershipDiscount,
          qrCodeType: payment.qrCodeType,
          // 🤖 Include AI verification fields
          receiptHash: payment.receiptHash,
          aiVerified: payment.aiVerified,
          aiConfidence: payment.aiConfidence,
          aiSuspicionScore: payment.aiSuspicionScore,
          aiRecommendation: payment.aiRecommendation,
          aiExtractedData: payment.aiExtractedData,
          aiRedFlags: payment.aiRedFlags,
          aiReasoning: payment.aiReasoning,
          aiVerifiedAt: payment.aiVerifiedAt,
          // 🆕 Reference number validation fields
          userReferenceNumber: payment.userReferenceNumber,
          referenceNumberMatch: payment.referenceNumberMatch
        };
      })
    );
      ...
      try {
        ...
        if (user) {
          ...
          const rejectionNotification = {
            ...
            date: new Date(),
            status: 'unread',
            message: `Payment rejected for ${coachName} class on ${updatedPayment.date} at ${updatedPayment.time}. Your booking has been cancelled and the schedule is now available again.`,
            timestamp: new Date(),
            read: false
          };
...
app.post('/api/book-package', async (req, res) => {
  try {
    const { userId, packageType, packageSessions, packagePrice, bookings, paymentProof, isPackage, userReferenceNumber } = req.body;
    ...
    if (!userReferenceNumber || userReferenceNumber.length !== 9) {
      return res.status(400).json({
        success: false,
        error: 'Valid 9-digit GCash reference number is required'
      });
    }
    ...
    // 🤖 AI RECEIPT VERIFICATION for Package Booking
    const AI_ENABLED = process.env.ENABLE_AI_VERIFICATION === 'true';
    const AI_TEST_MODE = process.env.AI_TEST_MODE !== 'false';
    ...
    if (AI_ENABLED) {
      try {
        console.log('🤖 [PACKAGE] AI verification enabled, analyzing receipt...');
        console.log('🤖 [PACKAGE] Test mode:', AI_TEST_MODE ? 'ON (logging only)' : 'OFF (enforcing)');

        aiVerification = await verifyReceiptWithAI(paymentProof, packagePrice, {
          userId: user.username,
          packageType,
          packageSessions,
          applicationType: 'package_booking'
        });

        console.log('🤖 [PACKAGE] AI Result:', {
          recommendation: aiVerification.recommendation,
          suspicionScore: aiVerification.suspicionScore,
          confidence: aiVerification.confidence,
          redFlags: aiVerification.redFlags
        ...
        // In TEST MODE: Log but don't enforce
        if (AI_TEST_MODE) {
          console.log('🧪 [PACKAGE] TEST MODE: AI result logged but not enforced');
          console.log('🧪 [PACKAGE] Would have:', aiVerification.recommendation);
          paymentStatus = 'for approval'; // Always pending in test mode
        } else {
          // PRODUCTION MODE: Enforce AI decision
          if (aiVerification.autoReject) {
            console.log('🚫 [PACKAGE] AI REJECTED: Auto-rejecting payment');
            return res.status(400).json({
              success: false,
              error: 'Receipt verification failed. Please upload a valid payment receipt.',
              details: aiVerification.redFlags,
          ...
          // Set status based on AI recommendation
          paymentStatus = aiVerification.suggestedStatus;
          console.log('✅ [PACKAGE] AI Status:', paymentStatus);
        }

      } catch (aiError) {
        console.error('❌ [PACKAGE] AI verification error:', aiError.message);
        // Continue with manual review if AI fails
        paymentStatus = 'for approval';
      }
    } else {
      console.log('ℹ️ [PACKAGE] AI verification disabled');
    }

    // 🆕 VALIDATE REFERENCE NUMBER MATCH for Package
    let referenceNumberMatch = false;
    if (aiVerification && userReferenceNumber) {
      const aiRefNumber = aiVerification.extractedData?.referenceNumber?.toString().trim();
      const userRefNumber = userReferenceNumber.toString().trim();

      console.log('🔍 [PACKAGE REF CHECK] User entered:', userRefNumber);
      console.log('🔍 [PACKAGE REF CHECK] AI extracted:', aiRefNumber);

      if (aiRefNumber && aiRefNumber === userRefNumber) {
        console.log('✅ [PACKAGE REF MATCH] Reference numbers match!');
        referenceNumberMatch = true;
        // Reduce suspicion score as reward for matching
        aiVerification.suspicionScore = Math.max(aiVerification.suspicionScore - 15, 0);
      } else if (aiRefNumber && aiRefNumber !== userRefNumber) {
        console.log('❌ [PACKAGE REF MISMATCH] Reference numbers do not match!');
        referenceNumberMatch = false;
        aiVerification.redFlags.push(`CRITICAL: Reference number mismatch - User entered ${userRefNumber}, receipt shows ${aiRefNumber}`);
        aiVerification.suspicionScore = Math.min(aiVerification.suspicionScore + 50, 100);
        ...
        console.log('⚠️ [PACKAGE REF MISMATCH] Status downgraded to manual review');
      } else {
        console.log('⚠️ [PACKAGE REF CHECK] AI could not extract reference number');
      }
    }

    // Check for duplicate receipts
    const duplicateCheck = await checkDuplicateReceipt(paymentProof);
    if (duplicateCheck.isDuplicate) {
      console.log('🚫 [PACKAGE] Duplicate receipt detected');
      return res.status(400).json({
        success: false,
        error: 'This receipt has already been used for another payment.',
        details: 'Please upload a different payment receipt.'
      });
    }
                  ...
                  <div style="max-width:520px;margin:40px auto;background:#fff;border-radius:8px;box-shadow:0 2px 16px rgba(0,0,0,0.07);overflow:hidden;">
                    ...
                    <div style="padding:36px 32px 32px 32px;text-align:center;">
                      ...
                      <div style="background:#f8f9fa;border-radius:8px;padding:20px;margin:20px 0;text-align:left;">
                        ...
                        <p style="margin:5px 0;color:#666;"><strong>Amount:</strong> ₱${parseFloat(updatedPayment.packagePrice).toFixed(2)}</p>
                      </div>
                      <a href="http://localhost:3000/my-schedule" style="display:inline-block;margin-top:18px;padding:14px 36px;background:#2ecc40;color:#fff;font-weight:bold;font-size:1.1rem;border-radius:6px;text-decoration:none;letter-spacing:1px;">View My Schedule</a>
                    </div>
                  </div>
                ...
                <div style="max-width:520px;margin:40px auto;background:#fff;border-radius:8px;box-shadow:0 2px 16px rgba(0,0,0,0.07);overflow:hidden;">
                  ...
                  <div style="padding:36px 32px 32px 32px;text-align:center;">
                    ...
                    <div style="background:#f8f9fa;border-radius:8px;padding:20px;margin:20px 0;text-align:left;">
                      ...
                      <p style="margin:5px 0;color:#666;"><strong>Reference Number:</strong> ${updatedPayment.referenceNumber}</p>
                      ...
                      <p style="margin:5px 0;color:#666;"><strong>Amount:</strong> ₱${parseFloat(updatedPayment.amount).toFixed(2)}</p>
                    </div>
                    <a href="http://localhost:3000/my-schedule" style="display:inline-block;margin-top:18px;padding:14px 36px;background:#2ecc40;color:#fff;font-weight:bold;font-size:1.1rem;border-radius:6px;text-decoration:none;letter-spacing:1px;">View My Schedule</a>
                  </div>
                </div>
    ...
    for (const booking of userBookingsWithPayments) {
      ...
      if (!exists) {
        allPayments.push({
          ...
          verificationDate: booking.verificationDate,
          referenceNumber: booking.referenceNumber, // Include reference number from booking
...
// Coach sends feedback to student
app.post('/api/coach/send-feedback', async (req, res) => {
  try {
    const {
      coachId,
      coachName,
    ...
    // Validate required fields
    if (!coachId || !coachName || !clientUsername || !feedback) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields'
      });
    ...
    // Find the user to send feedback to
    ...
    const feedbackNotification = {
      ...
      type: 'coach_feedback',
      message: `Coach ${coachName} sent you feedback: "${feedback}"`,
      timestamp: new Date(),
      read: false,
      feedbackData: {
        coachId,
    ...
    // Add notification to user
    user.notifications = user.notifications || [];
    user.notifications.push(feedbackNotification);
    await user.save();

    console.log(`✅ Feedback sent from coach ${coachName} to user ${clientUsername}`);

    res.json({
      success: true,
      message: 'Feedback sent successfully',
      notificationId: feedbackNotification._id
    });
...
app.post('/api/student/send-rating', async (req, res) => {
  try {
    const {
      studentUsername,
      studentName,
    ...
    // Validate required fields
    if (!studentUsername || !studentName || !coachId || !coachName || !rating) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields'
      });
    ...
    // Find the coach to send rating to
    const coach = await Coach.findById(coachId);
    if (!coach) {
      return res.status(404).json({
        success: false,
    ...
    const ratingNotification = {
      _id: new mongoose.Types.ObjectId(),
      type: 'student_rating',
      message: `${studentName} rated you ${rating} star${rating > 1 ? 's' : ''} for your ${bookingClass} class on ${new Date(bookingDate).toLocaleDateString()}.`,
      timestamp: new Date(),
      read: false,
      ratingData: {
        studentUsername,
    ...
    // Add notification to coach
    coach.notifications = coach.notifications || [];
    coach.notifications.push(ratingNotification);
    await coach.save();

    // Mark the booking as rated (update in booking history)
    try {
      // Find and update the booking in the user's booking history
      const user = await User.findOne({ username: studentUsername });
      if (user && user.bookingHistory) {
        const bookingIndex = user.bookingHistory.findIndex(booking =>
        ...
        if (bookingIndex !== -1) {
          user.bookingHistory[bookingIndex].feedbackGiven = true;
          user.bookingHistory[bookingIndex].rating = rating;
          user.bookingHistory[bookingIndex].ratingFeedback = feedback || '';
          user.bookingHistory[bookingIndex].ratedAt = new Date();
          await user.save();
        }
      }
    } catch (updateError) {
      console.error('Error updating booking history:', updateError);
      // Don't fail the request if we can't update the booking history
    }

    console.log(`✅ Rating sent from student ${studentName} to coach ${coachName}: ${rating} stars`);

    res.json({
      success: true,
      message: 'Rating sent successfully',
      notificationId: ratingNotification._id
    });
