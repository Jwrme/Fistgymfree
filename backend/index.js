      ...
      if (user) {
        ...
        user.bookings.forEach((booking, index) => {
          ...
          if (coachMatches && dateMatches && timeMatches) {
            ...
            hasUpdates = true;
            console.log(`✅ Updated booking status for ${user.username}: ${booking.coachName} (${booking.date}) to pending with reference: ${booking.referenceNumber}${isPackage ? ` (Package: ${packageType})` : ''}`);
          }
        });
