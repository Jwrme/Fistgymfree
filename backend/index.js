...
app.post('/api/updatePayment', async (req, res) => {
  try {
    ...
    const { userId, coachId, date, time, paymentProof, amount, originalAmount, membershipDiscount, hasMembershipDiscount, qrCodeType, className, userReferenceNumber, isMultiSlotBooking, multiSlotTotal, multiSlotCount } = req.body;
      ...
      if (user) {
        ...
        user.bookings.forEach((booking, index) => {
          ...
          if (coachMatches && dateMatches && timeMatches) {
            ...
            hasUpdates = true;
            console.log(`✅ Updated booking status for ${user.username}: ${booking.coachName} (${booking.date}) to pending with reference: ${booking.referenceNumber}`);
          }
        });
