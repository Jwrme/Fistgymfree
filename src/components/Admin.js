        ...
        <div style={{ background: '#f4f6f8', borderRadius: 8, padding: 24, textAlign: 'center' }}>
          ...
          <div style={{ color: '#222', fontSize: 16, marginBottom: 18 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 8, padding: 16, margin: '16px 0', textAlign: 'left' }}>
              ...
              <p style={{ margin: '4px 0', color: '#666' }}><strong>Amount:</strong> ₱500.00</p>
            </div>
          </div>
        </div>
      </div>
...
const Admin = () => {
  ...
  // Available specialties for coaches
  const AVAILABLE_SPECIALTIES = ['Kali', 'Boxing', 'MMA', 'Wrestling', 'Judo', 'Muay Thai', 'Jiu Jitsu Kids', 'Jiu Jitsu Adults'];
  ...
  // Toggle specialty selection for add coach
  ...
  // Toggle specialty selection for edit coach
  const toggleEditSpecialty = (specialty) => {
    setEditCoachSpecialties(prev =>
      prev.includes(specialty)
        ? prev.filter(s => s !== specialty)
        : [...prev, specialty]
    );
  ...
  const handleEditCoach = (coach) => {
    ...
    setEditCoachSpecialties(coach.specialties || []);
                ...
                <div style={{ padding: 20 }}>
                  <table className="admin-table" style={{ width: '100%', background: '#fff', borderRadius: 10, boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
                    ...
                    <tbody>
                      ...
                      {(() => {
                        // Group multiple session bookings by packageId
                        const groupPaymentsByPackage = (payments) => {
                          console.log('🔍 [ADMIN DEBUG] Grouping payments:', payments.length);
                          const packages = new Map();
                          const individualPayments = [];

                          payments.forEach((payment, index) => {
                            console.log(`🔍 [ADMIN DEBUG] Payment ${index + 1}:`, {
                              id: payment._id,
                              isPackage: payment.isPackage,
                            ...
                            if (payment.isPackage && payment.packageId) {
                              console.log(`✅ [ADMIN DEBUG] Grouping payment by packageId: ${payment.packageId}`);
                              // Group by packageId for multiple session bookings
                              if (!packages.has(payment.packageId)) {
                                packages.set(payment.packageId, {
                                  ...payment,
                                  sessions: [],
                                  totalAmount: 0,
                              ...
                              const packageGroup = packages.get(payment.packageId);
                              packageGroup.sessions.push(payment);
                              packageGroup.totalAmount += (payment.amount || 0);
                            } else {
                              console.log(`➡️ [ADMIN DEBUG] Adding as individual payment`);
                              // Individual payments or legacy packages
                              individualPayments.push(payment);
                            }
                          });

                          // Convert packages to array and combine with individual payments
                          const packageGroups = Array.from(packages.values());
                          console.log('🔍 [ADMIN DEBUG] Final result:', {
                            packageGroups: packageGroups.length,
                            individualPayments: individualPayments.length,
                            total: packageGroups.length + individualPayments.length
                          });
                          return [...packageGroups, ...individualPayments];
                        };

                        const groupedPayments = groupPaymentsByPackage(
                          pendingPayments.filter(payment => payment.status === 'for approval')
                        );

                        return groupedPayments.map(payment => (
                        <tr key={payment.isGroupedPackage ? payment.packageId : payment._id} style={{ background: payment.status === 'verified' ? '#eaffea' : payment.status === 'rejected' ? '#ffeaea' : '#fff' }}>
                          <td>{payment.isPackage ? (payment.isGroupedPackage ? 'Multi-Session Package' : 'Package Booking') : 'Booking'}</td>
                          ...
                          <td style={{
                            ...
                            {payment.isGroupedPackage ? 
                              payment.sessions.map(s => s.referenceNumber).filter(Boolean).join(', ') || 'N/A' :
                              payment.referenceNumber || 'N/A'
                            }
                          </td>
                          <td style={{
                            fontWeight: 'bold',
                            color: payment.userReferenceNumber ? '#2ecc40' : '#999',
                            fontSize: '13px',
                            fontFamily: 'monospace',
                            textAlign: 'center'
                          }}>
                            {payment.userReferenceNumber || 'N/A'}
                          </td>
                          <td>
                            {payment.isPackage ? (
                              <div>
                                <strong>{payment.packageType}</strong>
                                <br />
                                <small>{payment.isGroupedPackage ? payment.sessions.length : payment.packageSessions} sessions</small>
                              </div>
                            ) : (
                          ...
                          <td>
                            {payment.isGroupedPackage ? (
                              <div>
                                {/* Show all coaches involved in the package */}
                                {[...new Set(payment.sessions.map(s => s.coachName))].join(', ') || 'N/A'}
                              </div>
                            ) : (
                              payment.coachName || 'N/A'
                            )}
                          </td>
                          <td>
                            {payment.isGroupedPackage ? (
                              <div>
                                ...
                                <details style={{ fontSize: '12px', marginTop: '4px' }}>
                                  <summary style={{ cursor: 'pointer', color: '#17a2b8' }}>
                                    View {payment.sessions.length} sessions
                                  </summary>
                                  <div style={{ marginTop: '8px', padding: '8px', background: '#f8f9fa', borderRadius: '4px' }}>
                                    {payment.sessions.map((session, idx) => (
                                      <div key={idx} style={{ marginBottom: '4px', fontSize: '11px' }}>
                                        <strong>{session.coachName}</strong> - {new Date(session.date).toLocaleDateString()} at {session.time}
                                      </div>
                                    ))}
                                  </div>
                                </details>
                            ...
                            {payment.isGroupedPackage ? (
                              <div>
                                ...
                                <div style={{ fontSize: '11px', color: '#666' }}>
                                  ({payment.sessions.length} sessions × ₱{Math.round(payment.totalAmount / payment.sessions.length)})
                                </div>
                              </div>
                            ) : payment.isPackage ? (
                          ...
                          <td>
                            {payment.aiVerified ? (
                              <div style={{ fontSize: 12 }}>
                                {/* 🚨 CRITICAL WARNING for non-GCash receipts */}
                                {payment.aiRedFlags && payment.aiRedFlags.some(flag => 
                                  flag.includes('NOT A GCASH RECEIPT') || 
                                  flag.includes('NOT a valid GCash') ||
                                  flag.includes('Invalid/Not GCash')
                                ) && (
                                  <div style={{
                                    background: '#721c24',
                                    color: '#fff',
                                    ...
                                    🚨 NOT A GCASH RECEIPT!
                                    <div style={{ fontSize: 10, fontWeight: 'normal', marginTop: 4 }}>
                                      This image is not a valid GCash payment screenshot
                                    </div>
                                  </div>
                                )}
                                ...
                                <div style={{
                                  ...
                                  marginBottom: 6,
                                  background:
                                    payment.aiRecommendation === 'APPROVE' ? '#d4edda' :
                                      payment.aiRecommendation === 'REJECT' ? '#f8d7da' : '#fff3cd',
                                  color:
                                    payment.aiRecommendation === 'APPROVE' ? '#155724' :
                                      payment.aiRecommendation === 'REJECT' ? '#721c24' : '#856404',
                                  border: `1px solid ${payment.aiRecommendation === 'APPROVE' ? '#c3e6cb' :
                                    payment.aiRecommendation === 'REJECT' ? '#f5c6cb' : '#ffeeba'
                                    }`
                                }}>
                                  {payment.aiRecommendation === 'APPROVE' ? '✅ AI: APPROVE' :
                                    payment.aiRecommendation === 'REJECT' ? '❌ AI: REJECT' :
                                      '⚠️ AI: REVIEW'}
                                </div>
                                {/* AI Confidence */}
                                <div style={{ fontSize: 11, color: '#666', marginBottom: 2 }}>
                                  <strong>Confidence:</strong> {payment.aiConfidence}%
                                </div>
                                {/* Suspicion Score */}
                                <div style={{ 
                                  fontSize: 11, 
                                  color: payment.aiSuspicionScore >= 85 ? '#721c24' : payment.aiSuspicionScore >= 50 ? '#856404' : '#155724',
                                  marginBottom: 2,
                                  fontWeight: payment.aiSuspicionScore >= 85 ? 'bold' : 'normal'
                                }}>
                                  <strong>Risk:</strong> {payment.aiSuspicionScore}/100
                                  {payment.aiSuspicionScore >= 85 && ' 🔴'}
                                  {payment.aiSuspicionScore >= 50 && payment.aiSuspicionScore < 85 && ' 🟡'}
                                </div>
                                {/* Red Flags - Auto-expand if critical issues */}
                                {payment.aiRedFlags && payment.aiRedFlags.length > 0 && (
                                  <details 
                                    style={{ fontSize: 11, marginTop: 4 }}
                                    open={payment.aiRedFlags.some(flag => flag.includes('CRITICAL') || flag.includes('NOT A GCASH'))}
                                  >
                                    <summary style={{ cursor: 'pointer', color: '#e74c3c', fontWeight: 'bold' }}>
                                      🚩 {payment.aiRedFlags.length} Red Flag{payment.aiRedFlags.length > 1 ? 's' : ''}
                                    </summary>
                                    <ul style={{ margin: '4px 0', paddingLeft: 16, color: '#721c24' }}>
                                      {payment.aiRedFlags.map((flag, idx) => (
                                        <li key={idx} style={{ 
                                          fontWeight: flag.includes('CRITICAL') ? 'bold' : 'normal',
                                          color: flag.includes('CRITICAL') ? '#721c24' : '#856404'
                                        }}>
                                          {flag.includes('CRITICAL') && '⚠️ '}{flag}
                                        </li>
                                      ))}
                                    </ul>
                                  </details>
                                ...
                                {payment.userReferenceNumber && (
                                  <div style={{
                                    ...
                                    padding: '6px 10px',
                                    borderRadius: 6,
                                    background: payment.referenceNumberMatch ? '#d4edda' : '#f8d7da',
                                    border: `1px solid ${payment.referenceNumberMatch ? '#c3e6cb' : '#f5c6cb'}`
                                  }}>
                                    <div style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 2 }}>
                                      {payment.referenceNumberMatch ? '✅ Reference Match' : '❌ Reference Mismatch'}
                                    </div>
                                    <div style={{ fontSize: 10, color: '#666' }}>
                                      User entered: <strong>{payment.userReferenceNumber}</strong>
                                    </div>
                                    {payment.aiExtractedData?.referenceNumber && (
                                      <div style={{ fontSize: 10, color: '#666' }}>
                                        Receipt shows: <strong>{payment.aiExtractedData.referenceNumber}</strong>
                                      </div>
                                    )}
                                  </div>
                                )}
                              ...
                              <span style={{ color: '#999', fontSize: 11 }}>No AI check</span>
                            )}
                          </td>
                          <td style={{
                            fontWeight: 600,
                          ...
                          <td>
                            {payment.isGroupedPackage ? (
                              <div>
                                {/* Show payment proof from first session */}
                                {payment.sessions[0]?.paymentProof ? (
                                  <img
                                    src={payment.sessions[0].paymentProof}
                                    alt="Payment Proof"
                                    style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 6, border: '1px solid #eee', cursor: 'pointer' }}
                                    onClick={() => handleImageClick(payment.sessions[0].paymentProof)}
                                  />
                                ) : 'No proof'}
                                <div style={{ marginTop: 8, fontSize: 11, color: '#666' }}>
                                  Same proof for all {payment.sessions.length} sessions
                                </div>
                              </div>
                          ...
                          <td>
                            {payment.status === 'for approval' ? (
                              <>
                                <button
                                  onClick={() => {
                                    if (payment.isGroupedPackage) {
                                      // Approve all sessions in the package
                                      payment.sessions.forEach(session => {
                                        handlePaymentAction(session._id, 'approve');
                                      });
                                    } else {
                                      handlePaymentAction(payment._id, 'approve');
                                    }
                                  }}
                                  disabled={payment.isGroupedPackage ? 
                                    payment.sessions.some(s => actionLoading === s._id + 'approve') :
                                ...
                                >
                                  {payment.isGroupedPackage ? `Approve All (${payment.sessions.length})` : 'Approve'}
                                </button>
                                <button
                                  onClick={() => {
                                    if (payment.isGroupedPackage) {
                                      // Reject all sessions in the package
                                      payment.sessions.forEach(session => {
                                        handlePaymentAction(session._id, 'reject');
                                      });
                                    } else {
                                      handlePaymentAction(payment._id, 'reject');
                                    }
                                  }}
                                  disabled={payment.isGroupedPackage ? 
                                    payment.sessions.some(s => actionLoading === s._id + 'reject') :
                                ...
                                >
                                  {payment.isGroupedPackage ? `Reject All (${payment.sessions.length})` : 'Reject'}
                                </button>
                              </>
                            ) : (
                      ...
                      {pendingBookings.map(booking => (
                        <tr key={`booking-${booking._id}`}>
                          ...
                          <td>{booking.date}</td>
                          <td>{booking.amount ? `₱${parseFloat(booking.amount).toFixed(2)}` : 'N/A'}</td>
                          <td>
                          ...
                          <td>
                            ...
                            <button style={{ background: '#e74c3c', color: '#fff', marginRight: 8, border: 'none', borderRadius: 4, padding: '6px 14px', fontWeight: 600, cursor: 'pointer' }} onClick={() => handleBookingAction(booking._id, 'rejected')}>Reject</button>
                          </td>
                        </tr>
                      ))}
                      {(pendingPayments.filter(payment => payment.status === 'for approval').length === 0 && pendingBookings.length === 0) && (
                        <tr><td colSpan={9} style={{ textAlign: 'center', color: '#888', fontSize: 18 }}>No pending payments.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <section className="coaches-section">
                  ...
                  editingCoach === coach._id ? (
                    <div key={coach._id} className="edit-coach-form edit-card">
                      ...
                      <div style={{ width: '100%', marginBottom: 12 }}>
                        <label style={{ display: 'block', marginBottom: 8, color: '#666', fontSize: '0.85rem', fontWeight: '500' }}>
                          Classes/Specialties
                        </label>
                          ...
                          {AVAILABLE_SPECIALTIES.map(specialty => (
                            <button
                              ...
                              style={{
                                padding: '6px 12px',
                                border: '2px solid',
                                borderColor: editCoachSpecialties.includes(specialty) ? '#1ee43b' : '#ccc',
                                borderRadius: 6,
                                background: editCoachSpecialties.includes(specialty) ? '#1ee43b' : '#fff',
                                color: editCoachSpecialties.includes(specialty) ? '#fff' : '#333',
                                cursor: 'pointer',
                                fontSize: '0.8rem',
                                fontWeight: editCoachSpecialties.includes(specialty) ? 'bold' : 'normal',
              ...
              <div className="add-coach-form animated-card">
                ...
                <div style={{ width: '100%', marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 8, color: '#666', fontSize: '0.95rem', fontWeight: '500' }}>
                    Classes/Specialties
                  </label>
                    ...
                    {AVAILABLE_SPECIALTIES.map(specialty => (
                      <button
                        ...
                        style={{
                          padding: '8px 16px',
                          border: '2px solid',
                          borderColor: specialties.includes(specialty) ? '#1ee43b' : '#ccc',
                          borderRadius: 6,
                          background: specialties.includes(specialty) ? '#1ee43b' : '#fff',
                          color: specialties.includes(specialty) ? '#fff' : '#333',
                          cursor: 'pointer',
                          fontSize: '0.9rem',
                          fontWeight: specialties.includes(specialty) ? 'bold' : 'normal',
                          transition: 'all 0.2s ease',
                          fontFamily: 'Roboto Flex, sans-serif'
                        }}
                      >
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
                          <thead>
                            <tr style={{ background: '#f8f8f8' }}>
                              ...
                              <th style={{ padding: 8, fontWeight: 'bold', fontSize: 15 }}>Reference #</th>
                            </tr>
                          </thead>
                          <tbody>
                            {paymentHistory.length === 0 && (
                              <tr><td colSpan={7} style={{ textAlign: 'center', color: '#888' }}>No payment records found.</td></tr>
                            )}
                            {(() => {
                              ...
                              return currentPagePayments.map((rec, idx) => {
                                ...
                                return (
                                  <tr key={rec._id || idx}>
                                    ...
                                    <td style={{ padding: 8 }}>{rec.amount ? `₱${parseFloat(rec.amount).toFixed(2)}` : 'N/A'}</td>
