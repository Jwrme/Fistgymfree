...
const Membership = () => {
  ...
  const [referenceNumber, setReferenceNumber] = useState(''); // 🆕 GCash reference number
  ...
  // Stable onChange handler for reference number input
  ...
  // Stable style object for reference number input
  ...
  const handleSubmit = async (e) => {
    ...
    if (!referenceNumber || referenceNumber.length !== 9) {
      return alert('Please enter a valid 9-digit GCash reference number.');
    }
