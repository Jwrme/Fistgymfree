            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16 }}>
              <thead>
                <tr style={{ background: '#f8f8f8', color: '#222', fontWeight: 700 }}>
                  ...
                  <th style={{ padding: 10, borderBottom: '2px solid #eee' }}>Payment ReferenceNo.</th>
                  <th style={{ padding: 10, borderBottom: '2px solid #eee' }}>🤖 AI Recommendation</th>
              ...
              <tbody>
                {filteredApplications.length === 0 && (
                  <tr><td colSpan={10} style={{ textAlign: 'center', padding: 24, color: '#888' }}>No applications found.</td></tr>
                )}
                {currentApplications.map(app => (
                  <tr key={app._id} style={{ background: app.status === 'approved' ? '#eaffea' : app.status === 'rejected' ? '#ffeaea' : '#fff' }}>
                    ...
                    <td style={{ 
                      ...
                      fontWeight: 'bold', 
                      color: app.userReferenceNumber ? '#2ecc40' : '#999', 
                      fontSize: '13px',
                      fontFamily: 'monospace',
                      textAlign: 'center'
                    }}>
                      {app.userReferenceNumber || 'N/A'}
                    </td>
                      ...
                      {app.aiVerified ? (
                        <div style={{ fontSize: 12 }}>
                          ...
                          <div style={{
                            ...
                            marginBottom: 6,
                            background: 
                              app.aiRecommendation === 'APPROVE' ? '#d4edda' :
                              app.aiRecommendation === 'REJECT' ? '#f8d7da' : '#fff3cd',
                            color: 
                              app.aiRecommendation === 'APPROVE' ? '#155724' :
                              app.aiRecommendation === 'REJECT' ? '#721c24' : '#856404',
                            border: `1px solid ${
                              app.aiRecommendation === 'APPROVE' ? '#c3e6cb' :
                              app.aiRecommendation === 'REJECT' ? '#f5c6cb' : '#ffeeba'
                            }`
                          }}>
                            {app.aiRecommendation === 'APPROVE' ? '✅ APPROVE' :
                             app.aiRecommendation === 'REJECT' ? '❌ REJECT' :
                             '⚠️ REVIEW'}
                          </div>
                          {/* AI Confidence */}
                          <div style={{ fontSize: 11, color: '#666', marginBottom: 2 }}>
                            <strong>Confidence:</strong> {app.aiConfidence}%
                          </div>
                          {/* Suspicion Score */}
                          <div style={{ fontSize: 11, color: '#666', marginBottom: 2 }}>
                            <strong>Risk:</strong> {app.aiSuspicionScore}/100
                          </div>
                          {/* Red Flags */}
                          {app.aiRedFlags && app.aiRedFlags.length > 0 && (
                            <details style={{ fontSize: 11, marginTop: 4 }}>
                              <summary style={{ cursor: 'pointer', color: '#e74c3c', fontWeight: 'bold' }}>
                                🚩 {app.aiRedFlags.length} Red Flag{app.aiRedFlags.length > 1 ? 's' : ''}
                              </summary>
                          ...
                          {app.userReferenceNumber && (
                            <div style={{ 
                              marginTop: 8, 
                              padding: '6px 10px', 
                              borderRadius: 6,
                              background: app.referenceNumberMatch ? '#d4edda' : '#f8d7da',
                              border: `1px solid ${app.referenceNumberMatch ? '#c3e6cb' : '#f5c6cb'}`
                            }}>
                              <div style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 2 }}>
                                {app.referenceNumberMatch ? '✅ Reference Match' : '❌ Reference Mismatch'}
                              </div>
                              <div style={{ fontSize: 10, color: '#666' }}>
                                User entered: <strong>{app.userReferenceNumber}</strong>
                              </div>
                              {app.aiExtractedData?.referenceNumber && (
                                <div style={{ fontSize: 10, color: '#666' }}>
                                  Receipt shows: <strong>{app.aiExtractedData.referenceNumber}</strong>
                                </div>
                              )}
                            </div>
                          )}
