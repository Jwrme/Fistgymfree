...
// 2. ADD THESE FUNCTIONS after the openFeedbackModal function (around line 1885):
...
const handleRatingSubmit = async () => {
    ...
    try {
        const response = await fetch('http://localhost:3001/api/student/send-rating', {
            method: 'POST',
            ...
            body: JSON.stringify({
                ...
                coachName: selectedBookingForRating.coachName,
                bookingDate: selectedBookingForRating.date,
                bookingTime: selectedBookingForRating.time,
                bookingClass: selectedBookingForRating.class || selectedBookingForRating.className,
                rating: rating,
                feedback: ratingText.trim()
            })
        });
        ...
        if (data.success) {
            ...
            // Refresh booking history to show "Rated" status
            fetchBookingHistory();
        } else {
            alert('Failed to send rating: ' + (data.error || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error sending rating:', error);
        alert('Server error while sending rating.');
    } finally {
        setRatingSubmitting(false);
    }
};

// 3. REPLACE the completed bookings div (around line 3654-3668) with this:
...
// 4. ADD this rating modal component after the existing feedback modal (around line 2800):
        ...
        <div style={{
            ...
            {selectedBookingForRating && (
                <div style={{ 
                    ...
                    <strong>Coach:</strong> {selectedBookingForRating.coachName}<br/>
                    <strong>Class:</strong> {selectedBookingForRating.class || selectedBookingForRating.className}<br/>
                    <strong>Date:</strong> {new Date(selectedBookingForRating.date).toLocaleDateString()} at {selectedBookingForRating.time}
                </div>
            )}
            
            {/* Star Rating */}
            <div style={{ marginBottom: 16, textAlign: 'center' }}>
                <div style={{ fontSize: 16, marginBottom: 8, color: '#6c757d' }}>Rate your experience:</div>
            ...
            <textarea
                ...
                placeholder="Optional: Write a comment about your experience..."
                style={{
                    width: '100%',
                    height: 80,
                    padding: 12,
            ...
            <div style={{ 
                ...
                <button
                    ...
                    style={{
                        background: (rating > 0 && !ratingSubmitting) ? '#2ecc40' : '#ccc',
                        color: '#fff',
                        border: 'none',
                        borderRadius: 8,
                        padding: '10px 20px',
                        fontWeight: 'bold',
                        cursor: (rating > 0 && !ratingSubmitting) ? 'pointer' : 'not-allowed',
                        opacity: (rating > 0 && !ratingSubmitting) ? 1 : 0.6
                    }}
                >
