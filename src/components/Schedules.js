...
const Schedules = () => {
    ...
    const [packageStep, setPackageStep] = useState(1); // 1: coach selection, 2: date selection, 3: time slot selection
    ...
    // Get unique dates from coach's availability
    const getAvailableDates = (coachId, classType) => {
        const coachAvail = groupedAvailabilities[classType]?.find(avail => 
            avail.coach._id === coachId
        );
        
        if (!coachAvail) return [];
        
        const filteredAvail = coachAvail.availability.filter(slot => {
            if (!slot.class && classType === 'Boxing') return true;
            return (slot.class || '').toLowerCase() === classType.toLowerCase();
        });

        // Get unique dates and sort them
        const uniqueDates = [...new Set(filteredAvail.map(slot => slot.date))]
    ...
    const getTimeSlotsForDate = (coachId, classType, selectedDate) => {
        const coachAvail = groupedAvailabilities[classType]?.find(avail => 
            avail.coach._id === coachId
        );
        ...
        return coachAvail.availability.filter(slot => {
    ...
    const processPackagePayment = async (paymentProof) => {
                ...
                {showPackageModal && selectedPackageClass && (
                    <div className="package-modal" style={{
                        ...
                        <div style={{
                            ...
                            {packageStep === 2 && selectedPackageCoach && (
                                <>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 12, marginBottom: 20 }}>
                                        {getAvailableDates(selectedPackageCoach._id, selectedPackageClass).map((date, idx) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div style={{ fontSize: 14, fontWeight: 'bold', color: '#145a32' }}>{dayName}</div>
                                                    <div style={{ fontSize: 20, fontWeight: 'bold', margin: '4px 0' }}>{dayNumber}</div>
                                                    <div style={{ fontSize: 12, color: '#666' }}>{monthName}</div>
                                                </div>
                                            );
                                        })}
                                    </div>
                            ...
                            {packageStep === 3 && selectedPackageCoach && selectedDate && (
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                            ...
                                            <h4 style={{ color: '#145a32', margin: 0 }}>Step 3: Select Time Slots for {selectedDate && new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h4>
                                        </div>
                                    </div>
                                    <p style={{ color: '#666', marginBottom: 16, fontSize: 14 }}>
                                        Choose up to {membershipPackages[selectedPackageClass]?.sessions} available time slots with {selectedPackageCoach.firstname} {selectedPackageCoach.lastname} on {selectedDate && new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}.
                                    </p>
                                    ...
                                    <div style={{ maxHeight: 300, overflowY: 'auto', marginBottom: 20 }}>
                                        {(() => {
                                            ...
                                            // Only show time slots for the selected date
                                            const filteredAvail = coachAvail.availability.filter(slot => {
                                                if (slot.date !== selectedDate) return false;
                                                if (!slot.class && selectedPackageClass === 'Boxing') return true;
                                                return (slot.class || '').toLowerCase() === selectedPackageClass.toLowerCase();
