...
const UserProfile = () => {
  ...
  const submitRating = async (ratingData) => {
    try {
      ...
      // Update the class history to show that this booking has been rated
      setClassHistory(prevHistory => 
        prevHistory.map(classItem => 
          classItem.coachId === ratingData.coachId && 
          classItem.date === ratingData.date && 
          classItem.time === ratingData.time
            ? { ...classItem, hasRated: true }
            : classItem
        )
      );
  ...
  const handleRateCoach = (classItem) => {
    setSelectedBookingForRating(classItem);
