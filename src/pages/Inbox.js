...
const Inbox = () => {
  ...
  const handleFeedbackClick = (notif) => {
    console.log('🔍 Notification clicked:', notif);
    console.log('🔍 Notification type:', notif.type);
    console.log('🔍 Feedback data:', notif.feedbackData);
    
    if (notif.type === 'coach_feedback') {
      // Extract feedback data with fallback logic
      let feedbackData = notif.feedbackData || {};
      let coachName = feedbackData.coachName;
      let feedback = feedbackData.feedback;
      
      // Fallback: extract from notification message if feedbackData is incomplete
      if (!coachName && notif.message) {
        const coachMatch = notif.message.match(/Coach\s+([^s]+)\s+sent\s+you\s+feedback/);
        if (coachMatch) {
          coachName = coachMatch[1].trim();
          feedbackData.coachName = coachName;
        }
      }
      
      if (!feedback && notif.message) {
        // Try multiple patterns to extract feedback text
        // Pattern 1: "feedback: 'text'" or 'feedback: "text"'
        let feedbackMatch = notif.message.match(/feedback:\s*["']([^"']+)["']/);
        if (feedbackMatch) {
          feedback = feedbackMatch[1];
          feedbackData.feedback = feedback;
        } else {
          // Pattern 2: "sent you feedback: "text""
          feedbackMatch = notif.message.match(/sent\s+you\s+feedback:\s*["']([^"']+)["']/);
          if (feedbackMatch) {
            feedback = feedbackMatch[1];
            feedbackData.feedback = feedback;
          } else {
            // Pattern 3: Extract text between quotes after "feedback"
            feedbackMatch = notif.message.match(/feedback[^"']*["']([^"']+)["']/);
            if (feedbackMatch) {
              feedback = feedbackMatch[1];
              feedbackData.feedback = feedback;
            }
      ...
      console.log('✅ Opening feedback modal with data:', feedbackData);
      setSelectedFeedback(feedbackData);
      setShowFeedbackModal(true);
    } else {
      console.log('❌ Not a coach feedback notification');
    }
  };
    ...
    <div style={{ maxWidth: 600, margin: '80px auto 0 auto', background: 'none', padding: 0, minHeight: '60vh' }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {(notifications || []).map((notif, idx) => {
          ...
          // Special styling for coach feedback notifications
          const isCoachFeedback = notif.type === 'coach_feedback';
          const borderColor = isCoachFeedback ? '#007bff' : (!isRead(notif) ? '#2ecc40' : '#bbb');
          const bgColor = isCoachFeedback 
            ? (!isRead(notif) ? 'linear-gradient(135deg, #e8f4ff 90%, #f0f8ff 100%)' : 'linear-gradient(135deg, #f5f5f5 90%, #f0f0f0 100%)')
            : (!isRead(notif) ? 'linear-gradient(135deg, #eaffea 90%, #f8fff8 100%)' : 'linear-gradient(135deg, #f8f8f8 90%, #f4f4f4 100%)');
          const shadow = isCoachFeedback 
            ? (!isRead(notif) ? '0 4px 18px rgba(0,123,255,0.15)' : '0 2px 10px rgba(0,123,255,0.08)')
            : (!isRead(notif) ? '0 4px 18px rgba(46,204,64,0.13)' : '0 2px 10px rgba(44,204,64,0.07)');
          const statusColor = isCoachFeedback ? '#007bff' : (!isRead(notif) ? '#2ecc40' : '#888');
          return (
            <li key={notif._id || idx} style={{
              ...
              cursor: isCoachFeedback ? 'pointer' : 'default',
            }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: '#181818', fontSize: 16, textAlign: 'left', marginBottom: 2, letterSpacing: 0.5, wordBreak: 'break-word' }}>
                  {isCoachFeedback ? (
                    (() => {
                      // Extract feedback data with fallback logic
                      let coachName = notif.feedbackData?.coachName;
                      let feedback = notif.feedbackData?.feedback;
                      ...
                      return (
                        <>
                          {`Coach ${coachName || 'Unknown'} sent you feedback: "`}
                ...
                <span style={{
                  ...
                  padding: '3px 14px',
                  borderRadius: 16,
                  background: isRead(notif) ? '#eee' : statusColor,
                  color: isRead(notif) ? '#888' : '#fff',
                  fontWeight: 600,
                  fontSize: 13,
                  marginTop: 4,
                  letterSpacing: 0.5,
                  boxShadow: isRead(notif) ? 'none' : `0 1px 6px ${statusColor}33`,
                  border: isRead(notif) ? '1px solid #ddd' : 'none',
                }}>
                  {isCoachFeedback ? (isRead(notif) ? 'Coach Feedback' : 'New Feedback') : (isRead(notif) ? 'Read' : 'Unread')}
                </span>
              </div>
              <style>{`
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ marginBottom: 12 }}>
                <strong style={{ color: '#495057', fontSize: 16 }}>Coach:</strong>
                <span style={{ marginLeft: 8, fontSize: 16, color: '#007bff', fontWeight: 'bold' }}>
                  {selectedFeedback.coachName || 'Unknown'}
                </span>
              </div>
              {selectedFeedback.bookingClass && (
                <div style={{ marginBottom: 12 }}>
                  <strong style={{ color: '#495057', fontSize: 16 }}>Class:</strong>
                  <span style={{ marginLeft: 8, fontSize: 16, color: '#28a745', fontWeight: 'bold' }}>
                    {selectedFeedback.bookingClass}
                  </span>
                </div>
              )}
              {selectedFeedback.bookingDate && (
                <div>
                  <strong style={{ color: '#495057', fontSize: 16 }}>Session Date:</strong>
                  <span style={{ marginLeft: 8, fontSize: 16, color: '#6c757d' }}>
                    {new Date(selectedFeedback.bookingDate).toLocaleDateString('en-US', {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}{selectedFeedback.bookingTime ? ` at ${selectedFeedback.bookingTime}` : ''}
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{
              ...
              <p style={{
                ...
                "{selectedFeedback.feedback || 'No feedback text available'}"
              </p>
            </div>
