...
const Inbox = () => {
  ...
  const handleFeedbackClick = (notif) => {
    ...
    if (notif.type === 'coach_feedback' && notif.feedbackData) {
      console.log('✅ Opening feedback modal with data:', notif.feedbackData);
      setSelectedFeedback(notif.feedbackData);
      setShowFeedbackModal(true);
    } else {
      console.log('❌ Not a coach feedback notification or missing data');
    }
  };
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: '#181818', fontSize: 16, textAlign: 'left', marginBottom: 2, letterSpacing: 0.5, wordBreak: 'break-word' }}>
                  {isCoachFeedback ? (
                    <>
                      {`Coach ${notif.feedbackData?.coachName || 'Unknown'} sent you feedback: "`}
                      {notif.feedbackData?.feedback && notif.feedbackData.feedback.length > 50 
                        ? `${notif.feedbackData.feedback.substring(0, 50)}...` 
                        : notif.feedbackData?.feedback || 'No feedback text'
                      }
                      {'"'}
                      <br />
                      <span 
              ...
              <div>
                ...
                <span style={{ marginLeft: 8, fontSize: 16, color: '#6c757d' }}>
                  {new Date(selectedFeedback.bookingDate).toLocaleDateString('en-US', {
                    ...
                    month: 'long',
                    day: 'numeric'
                  })} at {selectedFeedback.bookingTime}
                </span>
              </div>
            </div>
